...
sicnv_unsup <- infercnv::run(sicnv_obj,
                            ...
                            out_dir = paste0(output_dir, "/", sample_name, "/sicnv_no_ref_unsup"),
                            cluster_by_groups = F,
                            HMM = F,
                            denoise = T,
...
all_obs <- read.dendrogram(file = paste0(output_dir, "/", sample_name, "/sicnv_no_ref_unsup/infercnv.observations_dendrogram.txt"))

all_obs_phylo <- as.phylo(all_obs)

png(paste0(output_dir, "/", sample_name, "/sicnv_no_ref_unsup/phylo_Nodes.png"),width=10000,height=2500, res = 300)
...
all_obs <- read.dendrogram(file = paste0(output_dir, "/", sample_name, "/sicnv_no_ref_sup/infercnv.observations_dendrogram.txt"))

all_obs_phylo <- as.phylo(all_obs)

png(paste0(output_dir, "/", sample_name, "/sicnv_no_ref_sup/phylo_Nodes.png"),width=10000,height=2500, res = 300)
plot(all_obs_phylo,show.tip.label = FALSE)
nodelabels(text=1:all_obs_phylo$Nnode,node=1:all_obs_phylo$Nnode+Ntip(all_obs_phylo))
dev.off()
