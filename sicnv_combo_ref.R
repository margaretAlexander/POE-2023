...
sicnv_combo_ref_out_unsup <- infercnv::run(sicnv_combo_ref,
                                     ...
                                     out_dir = paste0(out_dir, "/", obs_sample, "/sicnv/combo_ref_unsup"),
                                     cluster_by_groups = F,
                                     HMM = F,
                                     denoise = T,
...
all_obs <- read.dendrogram(file = paste0(out_dir, "/",obs_sample, "/sicnv/combo_ref_unsup/infercnv.observations_dendrogram.txt"))

all_obs_phylo <- as.phylo(all_obs)

png(paste0(out_dir, "/",obs_sample, "/sicnv/combo_ref_unsup/phylo_Nodes.png"),width=10000,height=2500, res = 300)
...
sicnv_combo_ref_out_sup <- suppressWarnings(infercnv::run(sicnv_combo_ref,
                                     cutoff = 0.1, # use 0.1 for 10x data
                                     out_dir = paste0(out_dir, "/",obs_sample, "/sicnv/combo_ref_sup"),
                                     cluster_by_groups = F,
                                     HMM = T,
                                     denoise = T,
...
all_obs <- read.dendrogram(file = paste0(out_dir, "/",obs_sample, "/sicnv/combo_ref_sup/infercnv.observations_dendrogram.txt"))

all_obs_phylo <- as.phylo(all_obs)

png(paste0(out_dir, "/",obs_sample, "/sicnv/combo_ref_sup/phylo_Nodes.png"),width=10000,height=2500, res = 300)
plot(all_obs_phylo,show.tip.label = FALSE)
nodelabels(text=1:all_obs_phylo$Nnode,node=1:all_obs_phylo$Nnode+Ntip(all_obs_phylo))
dev.off()
